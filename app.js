...
async function saveToSupabase() {
    ...
    const result = await supabaseFetch('POST', [
        { key: 'meta',   value: meta   },
...
function saveCatalog() {
    ...
    // ذخیره آنلاین روی Supabase - برای دیده شدن توسط همه مشتریان
...
function loadCatalog() {
    ...
    // بعد از Supabase آپدیت بگیر (آنلاین - برای همه مشتریان همون لحظه)
    if (navigator.onLine) {
        loadFromSupabase().then(ok => {
            if (ok) {
                applyTheme();
                renderCurrentView(false);
                console.log('✅ بارگذاری از Supabase موفق - صفحه آپدیت شد');
            } else {
                console.error('❌ بارگذاری از Supabase ناموفق:', window.__lastSupabaseError);
            }
        });
    } else {
        console.warn('⚠️ آفلاین - بارگذاری از Supabase انجام نشد');
    }
}
        ...
        if (gi < CATALOG_DATA.groups.length) {
            ...
            if (g.hasSubgroups) {
                        ...
                        if (ls.products) {
                            ls.products.forEach((lp, pi) => {
                                if (pi < (g.subgroups[si].products||[]).length) {
                                    _mergeProduct(g.subgroups[si].products[pi], lp);
                ...
                (lg.products || []).forEach((lp, pi) => {
                    if (pi < (g.products||[]).length) _mergeProduct(g.products[pi], lp);
...
/* مرج کامل و عمیق یک محصول - تضمین می‌کند رنگ‌ها، عکس‌ها، قیمت، توضیحات همه به‌درستی جایگزین شوند */
function _mergeProduct(target, source) {
    target.name            = source.name            ?? target.name;
    target.price           = source.price            ?? target.price;
    target.discountPercent = source.discountPercent  ?? target.discountPercent;
    target.cartonQty       = source.cartonQty        ?? target.cartonQty;
    target.description     = source.description      ?? target.description;
    target.image1          = source.image1           !== undefined ? source.image1 : target.image1;
    target.image2          = source.image2           !== undefined ? source.image2 : target.image2;
    target.colorType       = source.colorType        ?? target.colorType;
    if (source.colors) {
        target.colors = Object.assign({}, target.colors || {}, source.colors);
    }
}
