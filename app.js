    ...
    bar.innerHTML = `
        <div class="market-items">
            <div class="market-item">
                ...
                <span class="market-val" id="mDollar">${marketPrices.dollar || '—'}</span>
            </div>
            <div class="market-item">
                <span class="market-label">🥇 طلا</span>
                <span class="market-val" id="mGold">${marketPrices.gold || '—'}</span>
            </div>
            <div class="market-item">
                <span class="market-label">🔶 مس</span>
                <span class="market-val" id="mCopper">${marketPrices.copper || '—'}</span>
            </div>
            <div class="market-item">
                <span class="market-label">⚙️ آلومینیوم</span>
                <span class="market-val" id="mAluminum">${marketPrices.aluminum || '—'}</span>
            </div>
        </div>
    `;
...
function fetchMarketPrices() {
    // تلاش برای دریافت آنلاین از navasan.ir (CORS-friendly)
    fetch('https://api.navasan.tech/latest/?api_key=freekey&item=usd_sell,gold_18,copper,aluminum')
        .then(r => r.json())
        .then(data => {
            if (data) {
                marketPrices.dollar   = data.usd_sell?.value   ? formatPrice(data.usd_sell.value)   : marketPrices.dollar;
                marketPrices.gold     = data.gold_18?.value    ? formatPrice(data.gold_18.value)     : marketPrices.gold;
                marketPrices.copper   = data.copper?.value     ? formatPrice(data.copper.value)      : marketPrices.copper;
                marketPrices.aluminum = data.aluminum?.value   ? formatPrice(data.aluminum.value)    : marketPrices.aluminum;
                marketPrices.lastUpdate = nowJalali();
                saveMarketPrices();
                updateMarketDisplay();
            }
        })
        .catch(() => { /* اگر آنلاین نبود، مقادیر ذخیره‌شده نمایش داده می‌شن */ });
}
...
function updateMarketDisplay() {
    ...
    for (const [id, key] of Object.entries(fields)) {
        ...
        if (el) el.textContent = marketPrices[key] || '—';
    }
}
...
function saveCatalog() {
    try {
        const data = JSON.stringify(CATALOG_DATA);
        // بررسی حجم - localStorage معمولاً 5MB دارد
        const sizeKB = Math.round((data.length * 2) / 1024);
        if (sizeKB > 4500) {
            console.warn('⚠️ حجم داده نزدیک به حد مجاز:', sizeKB, 'KB');
        }
        localStorage.setItem('catalog_data', data);
        // تأیید ذخیره
        const check = localStorage.getItem('catalog_data');
        if (!check) {
            alert('⚠️ خطا در ذخیره‌سازی! حافظه مرورگر پر است.');
        }
    } catch(e) {
        console.error('خطا در ذخیره:', e);
        if (e.name === 'QuotaExceededError') {
            alert('⚠️ حافظه مرورگر پر است. لطفاً تعداد عکس‌ها را کم کنید یا از یک مرورگر دیگر استفاده کنید.');
        }
    }
}
...
function loadCatalog() {
    ...
    try {
        ...
        /* ---- groups ---- */
        if (!loaded.groups || !loaded.groups.length) return;

        loaded.groups.forEach((lg, gi) => {
            if (gi < CATALOG_DATA.groups.length) {
                const g = CATALOG_DATA.groups[gi];
                // عنوان و آیکون گروه
                if (lg.title) g.title = lg.title;
                if (lg.icon  !== undefined) g.icon  = lg.icon;

                if (g.hasSubgroups) {
                    (lg.subgroups || []).forEach((ls, si) => {
                        if (si < (g.subgroups || []).length) {
                            // بازیابی کامل زیرگروه
                            if (ls.title) g.subgroups[si].title = ls.title;
                            if (ls.icon !== undefined) g.subgroups[si].icon = ls.icon;
                            if (ls.products && ls.products.length) {
                                // بازیابی کامل هر محصول شامل عکس‌ها
                                ls.products.forEach((lp, pi) => {
                                    if (pi < (g.subgroups[si].products || []).length) {
                                        Object.assign(g.subgroups[si].products[pi], lp);
                                    } else {
                                        g.subgroups[si].products.push(lp);
                                    }
                                });
                            }
                        } else {
                    ...
                    // گروه مستقیم بدون زیرگروه
                    (lg.products || []).forEach((lp, pi) => {
                        if (pi < (g.products || []).length) {
                            Object.assign(g.products[pi], lp);
                        } else {
                            g.products.push(lp);
                        }
                    });
                ...
                // گروه جدید اضافه شده از پنل
                CATALOG_DATA.groups.push(lg);
            }
        });
    } catch(e) {
        console.error('خطا در بارگذاری کاتالوگ:', e);
    }
}
...
function renderAdminPage() {
    ...
    tabSettings.innerHTML = `
        ...
        <div class="admin-section">
            ...
            <p class="admin-hint">هر بار که قیمت‌ها یا محصولات را تغییر دادید، ورژن را یک عدد بالاتر ببرید و ذخیره کنید. مشتریان بعد از اتصال به اینترنت به‌صورت خودکار آپدیت می‌گیرند.</p>
            ...
            <p class="admin-hint" style="margin-top:4px">⚠️ پس از انتشار، همه مشتریان هنگام اتصال به اینترنت آپدیت می‌گیرند.</p>
        </div>
        <div class="admin-section">
    ...
    setTimeout(() => {
        ...
        document.getElementById('btnPublishUpdate')?.addEventListener('click', () => {
            ...
            saveCatalog();
            alert(`✅ ورژن ${ver} منتشر شد!\nمشتریان پس از اتصال به اینترنت بروزرسانی می‌گیرند.`);
        });
