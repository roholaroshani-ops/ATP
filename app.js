...
async function supabaseFetch(method, body) {
    try {
        ...
        if (!res.ok) {
            ...
            console.warn('Supabase HTTP error:', res.status, err);
            return null;
        }
...
async function loadFromSupabase() {
    ...
    try {
        ...
        if (metaRow)   Object.assign(CATALOG_DATA.meta, JSON.parse(metaRow.value));
        if (groupsRow) {
            const loaded = JSON.parse(groupsRow.value);
            loaded.forEach((lg, gi) => {
...
function saveCatalog() {
    ...
    // ذخیره آنلاین روی Supabase
    saveToSupabase();
}

function loadCatalog() {
    // اول از localStorage (سریع - آفلاین)
    const saved = localStorage.getItem('catalog_data');
    if (saved) {
        try {
            const loaded = JSON.parse(saved);
            if (loaded.meta) Object.assign(CATALOG_DATA.meta, loaded.meta);
            if (loaded.groups) _mergeGroups(loaded.groups);
        } catch(e) { console.log('localStorage error:', e); }
    }
    // بعد از Supabase آپدیت بگیر (آنلاین - برای همه مشتریان)
    if (navigator.onLine) {
        loadFromSupabase().then(ok => {
            if (ok) { applyTheme(); renderCurrentView(false); }
        });
    }
}

function _mergeGroups(loadedGroups) {
    loadedGroups.forEach((lg, gi) => {
        if (gi < CATALOG_DATA.groups.length) {
            ...
            if (g.hasSubgroups) {
                    ...
                    if (si < (g.subgroups||[]).length) {
                        ...
                        if (ls.products) {
                            ls.products.forEach((lp, pi) => {
                                if (pi < (g.subgroups[si].products||[]).length) {
                                    Object.assign(g.subgroups[si].products[pi], lp);
                                } else { g.subgroups[si].products.push(lp); }
                            });
                        }
                    } else {
                        if (!g.subgroups) g.subgroups = [];
                ...
                (lg.products || []).forEach((lp, pi) => {
                    if (pi < (g.products||[]).length) Object.assign(g.products[pi], lp);
                    else g.products.push(lp);
                });
            }
        } else {
            CATALOG_DATA.groups.push(lg);
